<?php

namespace Whitecube\LaravelPreset\Components\Publishers;

use Whitecube\LaravelPreset\Components\File;
use Whitecube\LaravelPreset\Components\FilesCollection;
use Whitecube\LaravelPreset\Components\PublisherInterface;

class Form implements PublisherInterface
{
    /**
     * Get the component's displayable name.
     */
    public function label(): string
    {
        return 'Form';
    }

    /**
     * Let the publisher prompt for eventual extra input
     * and return a collection of publishable files.
     */
    public function handle(): FilesCollection
    {
        $style = File::makeFromStub(
            stub: 'components/form/style.scss',
            destination: resource_path('sass/parts/_form.scss'),
        );

        $view = File::makeFromStub(
            stub: 'components/form/view.blade.php',
            destination: resource_path('views/components/form.blade.php'),
        );

        $component = File::makeFromStub(
            stub: 'components/form/Component.php',
            destination: base_path('app/View/Components/Form.php'),
        );

        return FilesCollection::make([
            $style,
            $view,
            $component,
        ]);
    }

    /**
     * Get the component's usage instructions
     */
    public function instructions(): ?string
    {
        return "1. Add `@import 'parts/form';` to `resources/sass/app.scss`\r\n2. Use the blade component: `<x-form tag=\"a\" href=\"/example\" modifier=\"small\" icon=\"arrow-right\">Content</x-form>`.\r\nBy default the component's tag is a \"form\". If it's an `a`, it should also have an `href` attribute.\r\n3. You should add the possible icons to the \$availabl-icons variable at the beggining of the 'parts/_button.scss' file.";
    }
}
